...
# 第1章 绪论
## 1.1 课题背景
互联网发展至今，无论是其理论还是技术都已经成熟，而且它广泛参与在社会中的方方面面。它让信息都可以通过网络传播，搭配信息管理工具可以很好地为人们提供服务。所以各行业，尤其是规模较大的企业和学校等都开始借助互联网和软件工具管理信息，传播信息，共享信息等等，以此可以增强自身实力，提高在同行业当中的竞争能力，并从各种激烈的竞争中获取发展的机会。针对高校教师成果信息管理混乱，出错率高，信息安全性差，劳动强度大，费时费力等问题，经过分析和考虑，在目前的情况下，可以引进一款小说阅读系统这样的现代化管理工具，这个工具就是解决上述问题的最好的解决方案。它不仅可以实时完成信息处理，还缩短高校教师成果信息管理流程，使其系统化和规范化。同时还可以减少工作量，节约高校教师成果信息管理需要的人力和资金。所以小说阅读系统是信息管理环节中不可缺少的工具，它对管理者来说非常重要。
## 1.2 课题意义 
现如今，信息种类变得越来越多，信息的容量也变得越来越大，这就是信息时代的标志。近些年，计算机科学发展得也越来越快，而且软件开发技术也越来越成熟，因此，在生活中的各个领域，只要存在信息管理，几乎都有计算机的影子，可以说很多行业都采用计算机的方式管理信息。信息计算机化处理相比手工操作，有着保密性强，效率高，存储空间大，成本低等诸多优点。针对高校教师成果信息管理，采用小说阅读系统可以有效管理，使信息管理能够更加科学和规范。

总之，在实际中使用小说阅读系统，其意义如下：

第一点：小说阅读系统的实际运用，可以帮助管理人员在短时间内完成信息处理工作；
...
第四点：可以降低信息管理成本；
## 1.3 研究内容
对小说阅读系统设计制作，不仅需要技术支撑，也需要大量的理论研究。本文在对小说阅读系统进行介绍时，将按照如下内容进行。

第一部分：介绍小说阅读系统研究的背景意义，便于用户了解系统；

第二部分：介绍开发小说阅读系统需要搭建的环境，包括技术和工具；

第三部分：介绍用户对小说阅读系统的功能要求，以及对小说阅读系统的性能要求等；
...
# 第2章 开发环境与技术
小说阅读系统的编码实现需要搭建一定的环境和使用相应的技术，接下来的内容就是对小说阅读系统用到的技术和工具进行介绍。
## 2.1 MYSQL数据库
...
### 3.1.2 经济可行性
本系统开发期间需要配置的软件环境，可以免费通过开发类官网下载安装，需要配置的硬件设备也不需要具备很高的性能，通常网吧电脑，或学校计算机机房的电脑都符合要求。因此，从经济方面考虑，小说阅读系统开发可行。
### 3.1.3 操作可行性
小说阅读系统根据用户使用习惯进行开发，设计的界面具有统一性，并具备优秀的导航功能。所以，只要会简单操作电脑的人员，可以无压力操作小说阅读系统。
...
关系：关系就是数据库中的一张数据表，每张数据表都有命名，也就是每个关系也有名字，那就是数据表名；元组：元组就是数据表中的一行记录；属性：属性就是数据表中的字段，也就是数据表中的一列；域：域就是对数据表中属性的取值进行限定；关键字：关键字就是数据表中的主键。系统的各实体信息的实体属性图，以及总体E-R图如下所示。
...
| :-: | :-: | :-: | :-: | :-: |
|1|id|Int|编号|否|
|2|xiaoshuo\_name|String|作品名称|是|
|3|xiaoshuo\_photo|String|作品封面|是|
|4|xiaoshuo\_zuozhe|String|作者|是|
|5|xiaoshuo\_zishu|Integer|字数|是|
|6|xiaoshuo\_types|Integer|作品类型|是|
|7|xiaoshuo\_clicknum|Integer|点击次数|是|
|8|shangxia\_types|Integer|是否上架|是|
|9|xiaoshuo\_delete|Integer|逻辑删除|是|
|10|xiaoshuo\_content|String|作品简介|是|
...
|2|xiaoshuo\_id|Integer|作品|是|
|3|yonghu\_id|Integer|用户|是|
|4|xiaoshuo\_liuyan\_text|String|评论内容|是|
...
图5.1 用户信息管理页面
### 5.2 作品信息管理
如图5.2显示的就是作品信息管理页面，此页面提供给管理员的功能有：查看已发布的作品信息数据，修改作品信息，作品信息作废，即可删除，还进行了对作品信息名称的模糊查询 作品信息信息的类型查询等等一些条件。
...
图5.2 作品信息管理页面
### 5.3作品类型管理
如图5.3显示的就是作品类型管理页面，此页面提供给管理员的功能有：根据作品类型进行条件查询，还可以对作品类型进行新增、修改、查询操作等等。
